...
object TomlGenerator {
  ...
  def apply(mod: Module, coverage: Seq[(WRef,Statement)]) = {
    ...
    println("# This file was generated by the AutoCoverage pass.")
    println("# It contains all information needed by the fuzzer and")
    println("# to generate the FPGA as well as the C++ test harness.")
    println(s"""[general]""")
    ...
    for((cond, stmt) <- coverage) {
      ...
      for(inverted <- Seq(false, true)) {
        ...
        println(s"""counter-bits = 1""")
        println(s"""filename = "${dbg.filename}"""")
        println(s"""line = ${dbg.line}""")
        println(s"""column = ${dbg.col}""")
        println(s"""human-expr = "${human}"""")
        println()
        index += 1
      }
    }
