...
impl TestHistory {
	...
	fn new_test(&mut self, info: &MutationInfo, slot: &BufferSlot) -> TestId {
		self.mutation.new_test(info, self.test_id);
		self.buffer.new_test(slot, self.test_id);
		self.test_id = self.test_id.next();
		self.test_id
	}
	fn get_mutation_info(&mut self, id: TestId) -> MutationInfo {
		self.mutation.get_info(id, self.test_id)
	}
	fn get_buffer_slot(&mut self, id: TestId) -> BufferSlot {
