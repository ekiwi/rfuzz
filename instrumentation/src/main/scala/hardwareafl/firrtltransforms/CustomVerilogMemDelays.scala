...
// See https://github.com/freechipsproject/firrtl/issues/547
// This is also modified to run on MidForm
class CustomVerilogMemDelays extends Transform {
  ...
  def execute(state: CircuitState): CircuitState = {
    state.copy(circuit = state.circuit.copy(modules = state.circuit.modules.map(memDelayMod)))
  }
}
